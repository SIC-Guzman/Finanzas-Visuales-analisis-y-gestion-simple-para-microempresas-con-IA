...
def process_analysis(filename):
    ...
    try:
        ...
        try:
            ...
            resultados['resultados_ia'] = {'error': f"No se pudieron generar predicciones de IA: {str(e)}"}
            print("⚠️ DEBUG: Error en IA ventas/costos:", e)
        ...
        try:
            ...
            resultados["anomalias_financieras"] = {"error": f"Error en Anomalias Financieras: {str(e)}"}
            print("⚠️ DEBUG: Error en anomalias:", e)
        ...
        try:
            gen = GeneradorInsights(
                ...
                horizontal = resultados.get('resultados', {}).get('horizontal', {}),
                vertical = resultados.get('resultados', {}).get('vertical', {}),
                predicciones = resultados.get('resultados_ia', []),
                anomalias = resultados.get('anomalias_financieras', {})
            )
            insights_obj = gen.generar_insights()

            if isinstance(insights_obj, dict):
                resultados.setdefault('insights', insights_obj.get('insights', []))
                resultados['resumen'] = insights_obj.get('resumen', resultados.get('resumen', {}))
                resultados.setdefault('recomendaciones', insights_obj.get('recomendaciones', []))
            else:
                resultados['insights'] = insights_obj if isinstance(insights_obj, list) else []
                if not resultados.get('resumen'):
                    titulos = [i.get('titulo') for i in resultados['insights'][:4] if isinstance(i, dict) and i.get('titulo')]
                    resultados['resumen'] = "Puntos clave: " + "; ".join(titulos) if titulos else "Resumen no disponible"
        except Exception as e:
            resultados["insights"] = [{
                "nivel": "error",
                "titulo": "No se pudieron generar insights",
                "recomendacion": str(e)
            }]
            if not resultados.get('resumen'):
                resultados['resumen'] = "Resumen no disponible por error en generador de insights."
            print("⚠️ DEBUG: Error generando insights:", e)

        # --- Convertir dict a DotDict para usar .resumen, .insights, etc ---
        class DotDict(dict):
            __getattr__ = dict.get
            __setattr__ = dict.__setitem__

        resultados = DotDict(resultados)
        if isinstance(resultados.resumen, dict):
            resultados.resumen = DotDict(resultados.resumen)
        if isinstance(resultados.get('resultados', {}), dict):
            resultados.resultados = DotDict(resultados.resultados)
